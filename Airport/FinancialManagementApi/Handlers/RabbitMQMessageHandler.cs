...
using FinancialManagementApi.Events;
        ...
        public RabbitMQMessageHandler(IServiceCollection services)
        {
            ...
            while (_config[$"RabbitMQHandler:RoutingKeys:{index}:RoutingKey"] != null)
            {
                ...
                consumer.Received += (model, ea) =>
                {
                    ...
                    var repo = serviceProvider.GetService<IFinancialEventRepository>();
                    var repo2 = serviceProvider.GetService<IFinancialRepository>();

                    var type = Type.GetType($"FinancialManagementApi.Events.{routingKey}");
                    IEvent eventObj = (IEvent)Activator.CreateInstance(type, _config, _channel, repo,repo2);
