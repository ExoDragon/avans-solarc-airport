...
using FlightManagementApi.Events;
...
namespace FlightManagementApi.Handlers
{
    public class RabbitMQMessageHandler : IMessageHandler
    {
        ...
        public RabbitMQMessageHandler(IServiceCollection services)
        {
            ...
            var factory = new ConnectionFactory() { HostName = _config["RabbitMQHandler:Host"], Port = 5672, UserName = _config["RabbitMQHandler:Username"], Password = _config["RabbitMQHandler:Password"] };
            ...
            while (_config[$"RabbitMQHandler:RoutingKeys:{index}:RoutingKey"] != null)
            {
                ...
                consumer.Received += (model, ea) =>
                {
                    ...
                    var type = Type.GetType($"FlightManagementApi.Events.{routingKey}");
